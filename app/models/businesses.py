...
class Business(Base):
    ...
    # 업소 기본 정보
    business_code        = Column(String(30), unique=True, nullable=False)  # 상가업소번호 (bizesId)
    biz_name             = Column(String(100), nullable=False)              # 상호명 (bizesNm)
    branch_name          = Column(String(100), nullable=True)               # 지점명 (brchNm). 빈값→NULL

    # 업종 분류 (대/중/소)
    category_large_code  = Column(String(10),  nullable=True)  # 상권업종대분류코드 (indsLclsCd)
    category_large_name  = Column(String(30),  nullable=True)  # 상권업종대분류명 (indsLclsNm)
    category_medium_code = Column(String(20),  nullable=True)  # 상권업종중분류코드 (indsMclsCd)
    category_medium_name = Column(String(50),  nullable=True)  # 상권업종중분류명 (indsMclsNm)
    ...
    plot_sct_code        = Column(String(5),   nullable=True)  # 대지구분코드 (plotSctCd). 1=대지,2=산
    ...
    # 영업 상태
    open_status          = Column(String(10), nullable=False, server_default='영업중')  # API 미제공. 월별 수집 시 폐업 UPDATE

    # 시간 정보
    created_at           = Column(DateTime, server_default=func.now())  # 최초 적재 시각
    updated_at           = Column(DateTime, server_default=func.now(), onupdate=func.now())  # 마지막 갱신 시각
