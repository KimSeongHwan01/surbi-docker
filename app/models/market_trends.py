...
class MarketTrend(Base):
    ...
    )  # 상권변화지표 코드 (TRDAR_CHNGE_IX). HH/HL/LH/LL
    trend_grade_nm = Column(
        String(10), nullable=True
    )  # 상권변화지표명 (TRDAR_CHNGE_IX_NM)

    # 행정동 평균 영업/폐업 기간
    opr_sale_mt_avrg = Column(
        Integer, nullable=True
    )  # 행정동 평균 영업기간 개월 (OPR_SALE_MT_AVRG)
    cls_sale_mt_avrg = Column(
        Integer, nullable=True
    )  # 행정동 평균 폐업기간 개월 (CLS_SALE_MT_AVRG)

    # 서울시 전체 평균 (상대 비교용)
    su_opr_sale_mt_avrg = Column(
        Integer, nullable=True
    )  # 서울시 전체 평균 영업기간 (SU_OPR_SALE_MT_AVRG)
    su_cls_sale_mt_avrg = Column(
        Integer, nullable=True
    )  # 서울시 전체 평균 폐업기간 (SU_CLS_SALE_MT_AVRG)
